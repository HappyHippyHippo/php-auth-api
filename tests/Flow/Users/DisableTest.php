...
class DisableTest extends EndpointTester
{
    ...
    public function testUserNotFound(): void
    {
        $expectedBody = [
            'status' => [
                'success' => false,
                'errors' => [['code' => 's2.e205.c10', 'message' => 'user not found']],
            ],
        ];

        $this->userId = "1";
        $this->client->request('POST', '/users/1/disable', [], [], self::HEADERS);
    ...
    public function testSuccess(): void
    {
        ...
        $user = $fixtures->addUser(['enabled' => true]);
        $this->loadFixtures($fixtures);
        ...
        $this->client->request('POST', '/users/' . $this->userId . '/disable', [], [], self::HEADERS);
        $response = $this->client->getResponse();
        $responseBody = json_decode((string) $response->getContent(), true);
        ...
        $this->assertFalse($users[1]->isEnabled());
