...
class DisableTest extends EndpointTester
{
    ...
    public function testUserNotFound(): void
    {
        $expectedBody = [
            'status' => [
                'success' => false,
                'errors' => [['code' => 's2.e505.c10', 'message' => 'user not found']],
            ],
        ];

        $this->client->request(
            'POST',
            '/users/' . self::USER_ID . '/permissions/' . self::PERMISSION_ID . '/disable',
            [],
            [],
            self::HEADERS
        );
    ...
    public function testPermissionNotFound(): void
    {
        ...
        $expectedBody = [
            'status' => [
                'success' => false,
                'errors' => [['code' => 's2.e505.c27', 'message' => 'user permission not found']],
            ],
        ];
    ...
    public function testUpdatePermission(): void
    {
        ...
        $fixtures->addUser(['email' => 'root@email.com']);
        $user = $fixtures->addUser();
        $fixtures->addUserPermission(['user' => $user, 'enabled' => true]);
        $this->loadFixtures($fixtures);
