...
class DisableTest extends EndpointTester
{
    ...
    public function testRoleNotFound(): void
    {
        $expectedBody = [
            'status' => [
                'success' => false,
                'errors' => [['code' => 's2.e405.c23', 'message' => 'role not found']],
            ],
        ];

        $this->client->request(
            'POST',
            '/roles/' . self::ROLE_ID . '/permissions/' . self::PERMISSION_ID . '/disable',
            [],
            [],
            self::HEADERS
        );
    ...
    public function testPermissionNotFound(): void
    {
        ...
        $expectedBody = [
            'status' => [
                'success' => false,
                'errors' => [['code' => 's2.e405.c25', 'message' => 'role permission not found']],
            ],
        ];
    ...
    public function testUpdatePermission(): void
    {
        ...
        $role = $fixtures->addRole();
        $fixtures->addRolePermission(['role' => $role, 'enabled' => true]);
        $this->loadFixtures($fixtures);
        ...
        $this->assertFalse($perms[0]->isEnabled());
