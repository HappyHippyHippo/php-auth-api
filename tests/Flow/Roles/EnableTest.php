...
class EnableTest extends EndpointTester
{
    ...
    public function testRoleNotFound(): void
    {
        $expectedBody = [
            'status' => [
                'success' => false,
                'errors' => [['code' => 's2.e304.c23', 'message' => 'role not found']],
            ],
        ];

        $this->roleId = "1";
        $this->client->request('POST', '/roles/1/enable', [], [], self::HEADERS);
    ...
    public function testSuccess(): void
    {
        ...
        $role = $fixtures->addRole(['enabled' => false]);
        $this->loadFixtures($fixtures);
        ...
        $this->client->request('POST', '/roles/' . $this->roleId . '/enable', [], [], self::HEADERS);
        $response = $this->client->getResponse();
        $responseBody = json_decode((string) $response->getContent(), true);
